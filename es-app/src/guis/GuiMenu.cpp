    ...
    if (Utils::FileSystem::exists("/opt/system/Bluetooth.sh") && !std::string(getShOutput(R"(cat /sys/class/graphics/fb0/modes | grep -o -P '(?<=:).*(?=p-)' | cut -dx -f1 | grep 720)")).empty())
     {
        ...
        addEntry(_("WIFI") + ": " + std::string(getShOutput(R"(if [ -z $(rfkill -n -o TYPE,SOFT | grep wlan | grep -w unblocked) ]; then echo "Off"; else cat /sys/class/net/wlan0/operstate; fi)")), false, [this] {  });
     } else if (Utils::FileSystem::exists("/opt/system/Bluetooth.sh")) {
	    addEntry(_("BAT") + ": " + std::string(getShOutput(R"(cat /sys/class/power_supply/battery/capacity)")) + "%" + " | " + _("SND") + ": " + std::string(getShOutput(R"(current_volume)")) + " | " + _("BRT") + ": " + std::to_string(ApiSystem::getInstance()->getBrightnessLevel()) + "% | " + _("BT") + ": " + std::string(getShOutput(R"(if [ -z $(pidof rtk_hciattach) ]; then echo "Off"; else echo "On"; fi)")) + " | " + _("WIFI") + ": " + std::string(getShOutput(R"(if [ -z $(rfkill -n -o TYPE,SOFT | grep wlan | grep -w unblocked) ]; then echo "Off"; else cat /sys/class/net/wlan0/operstate; fi)")), false, [this] {  });
     } else {
	    addEntry(_("BAT") + ": " + std::string(getShOutput(R"(cat /sys/class/power_supply/battery/capacity)")) + "%" + " | " + _("SND") + ": " + std::string(getShOutput(R"(current_volume)")) + " | " + _("BRT") + ": " + std::to_string(ApiSystem::getInstance()->getBrightnessLevel()) + "% | " + _("WIFI") + ": " + std::string(getShOutput(R"(if [[ -z $(rfkill -n -o TYPE,SOFT | grep wlan | grep -w unblocked) ]]; then echo "Off"; else cat /sys/class/net/wlan0/operstate; fi)")), false, [this] {  });
     }
...
void GuiMenu::openThemeConfiguration(Window* mWindow, GuiComponent* s, std::shared_ptr<OptionListComponent<std::string>> theme_set,const std::string systemTheme)
{
	...
	if (systemTheme.empty())
	{
		...
		if (system != NULL)
		{
			...
			{
				if (it->first == "basic" || it->first == "detailed" || it->first == "video" || it->first == "grid")
